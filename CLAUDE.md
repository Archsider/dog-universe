...
| `no-inline-deletedAt-null` | `deletedAt: null` inline dans un `prisma.*.findFirst/findMany` where-clause sans passer par `notDeleted()` de `src/lib/prisma-soft.ts`. Warn (pas error) pour permettre les overrides `-- OK:` légitimes (write ops, client-side state). Livré 2026-05-19, PR #131 |
...
- ~~**Auth guards dupliqués**~~ : **RÉSOLU (PR #131, 2026-05-19)** — `requireRole()` déployé sur 33+ routes admin. Plus d'inline `if (!session?.user || session.user.role !== ...)`.
- ~~**`notDeleted()` non utilisé**~~ : **RÉSOLU (PR #131, 2026-05-19)** — règle ESLint `no-inline-deletedAt-null` (#9) + batch migration de toutes les occurrences inline sur crons + routes booking. Ancienne mention : 99 occurrences → désormais quasi-0 (seuls les escape comments `-- OK:` légitimes subsistent).
...
| Invariant `checkItemAllocatedOverflow` flag faux les DISCOUNT items | RÉSOLU (2026-05-15, PR Bug B) | **Constat** : l'invariant `WHERE "allocatedAmount" > total + 0.01` flag systématiquement tout item `category='DISCOUNT'` (total négatif par construction, allocatedAmount=0 par design). `0 > -150 + 0.01 = -149.99` → TRUE → faux positif sur toute facture remisée. **Cause** : règle SQL sans guard pour les items déductifs. La sémantique métier "tu ne peux pas allouer plus de cash à un item que son prix" est vacante pour les items à prix négatif. **Patch** : ajout `total > 0 AND` sur les 2 queries (sample + count) de `checkItemAllocatedOverflow` (`src/lib/health-invariants.ts:188-210`). Plus robuste qu'un filtre `category != 'DISCOUNT'` car insensible aux noms d'enum futurs. **Verify** : 4 tests régression dans `health-invariants.test.ts` — DISCOUNT item ignoré, vrai overflow toujours flag, guard SQL présent sur sample ET count. |
| 4 CHECK constraints DB integrity (Review.rating ∈ [1,5], Payment.amount ≠ 0, TimeProposal.{proposed,responded}ByRole enum) | LIVRÉ (2026-05-17, PR db-integrity-hardening) | Migration `20260518_data_integrity_hardening` (idempotente via DO blocks). Bonus : RGPD StayPhoto leak fixé sur 3 routes (filter `booking.deletedAt: null`), cap `Notification` backup 10k→100k + warning saturation. |
| 4 indexes hot path manquants (dashboard + slide-over + crons) | LIVRÉ (2026-05-17, PR perf-indexes-may17) | Audit perf a relevé 4 hot paths sans index dédié : (1) `Booking_capacity_partial_idx` partial sur `(status, startDate, endDate) WHERE isOpenEnded=false AND deletedAt IS NULL` (dashboard "Pension actuelle" + capacity overlap), (2) `Vaccination_nextDueDate_confirmed_idx` partial sur `(nextDueDate) WHERE status='CONFIRMED'` (widget dashboard "Vaccins à renouveler"), (3) `ActionLog_entity_createdAt_idx` sur `(entityType, entityId, createdAt DESC)` (section History du slide-over panel), (4) `Notification_user_type_date_idx` sur `(userId, type, createdAt DESC)` (5 crons dedup : reminders/birthday/contract-reminders/overdue/review-requests). Migration `20260518_perf_indexes` SANS `CONCURRENTLY` (incompatible avec le BEGIN/COMMIT wrapper de `db-migrate.mjs`) — lock ACCESS EXCLUSIVE < 1 s acceptable car tables small-medium. Partial indexes (Booking, Vaccination) gérés en SQL pur (Prisma ne supporte pas `WHERE` sur `@@index`) ; les 2 autres représentés dans `schema.prisma` via `@@index(..., name: ...)`. |
| `InvoiceItem.category='PRODUCT'` sans `productId` (defense in depth) | LIVRÉ (2026-05-18, PR product-category-hard-floor-may17) | 3 floors empilés : (1) ESLint `dog-universe/no-hardcoded-product-without-id` bloque le merge sur tout object literal InvoiceItem-shaped avec `category: 'PRODUCT'` et `productId` manquant/null. Heuristique "shape" : object doit avoir ≥ 2 des clés `description/quantity/unitPrice/total` pour fire (évite faux positifs sur dropdown templates). (2) Zod refine `PRODUCT_CATEGORY_REQUIRES_PRODUCT_ID` sur les 3 routes d'écriture (POST `/api/invoices`, POST `/api/admin/walkin-invoice`, PATCH `/api/invoices/[id]`) → 400 explicite. (3) DB CHECK constraint `InvoiceItem_product_category_has_productId` posée par migration `20260518_product_category_requires_product_id` — Postgres rejette toute insertion violante. STEP 1 de la migration tente de récupérer le `productId` via lookup catalogue (LONGEST product name match) ; STEP 2 ajoute la CHECK ; rows non récupérées sont downgradées en `category='OTHER'` avec audit prefix `[Auto-fix 2026-05-18] Original: PRODUCT without productId — `. Idempotent (DO block sur la constraint, UPDATE WHERE-guarded). |
...
- **1565 tests passing** en local (1596 incl. 31 skipped — 5 suites intégration sans `INTEGRATION_DATABASE_URL`, 26 E2E Playwright sans secrets)
