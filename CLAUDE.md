...
## 🚨 URGENCES OUVERTES — REPRENDRE ICI (2026-05-12 fin de session)

**Contexte** : la branche `claude/restore-markdown-files-vqucK` contient plusieurs fixes (5 commits depuis main). Le dernier push (`d6efcf9`) est **rouge sur Vercel** — l'utilisateur n'a pas pu coller l'erreur exacte avant de fermer la fenêtre.

### 1. ⛔ BUG CRITIQUE : 3 SMS dupliqués envoyés en production
L'utilisateur a reçu **3 SMS identiques** aujourd'hui ("Luigi vient demain"). Cause confirmée par lecture du code :

- `src/lib/queues/index.ts` : SMS queue config = **3 attempts, exponential backoff 5min** (`SMS_JOB_OPTIONS`)
- `src/workers/processors.ts` (mon commit `d6efcf9`) ajoute `if (!ok) throw new Error('SMS delivery failed for ...')`
- `src/lib/sms.ts` ligne 81-106 : `sendSMS()` retourne `true` sur succès, **throws** sur échec réseau, retourne `false` seulement si phone/env manquant
- **Mais** : `recordSmsSent` peut échouer silencieusement si la migration `20260512_sms_log` n'a pas été exécutée sur Supabase → `isSmsDedup` retourne false → BullMQ retry → 3 SMS

**Action immédiate à faire dans la prochaine session** :
1. **REVERT** la ligne `if (!ok) throw` dans `src/workers/processors.ts` (lignes 82-84) OU la remplacer par `if (!ok) { logger.warn(...); return; }` — ne pas throw, sinon BullMQ retry → duplicates
2. **Vérifier sur Supabase** que la migration `20260512_sms_log` a bien été appliquée :
   ```sql
   SELECT COUNT(*) FROM "SmsLog";  -- si erreur "relation does not exist" → migration non lancée
   SELECT indexname FROM pg_indexes WHERE tablename = 'SmsLog';  -- doit contenir SmsLog_phone_contentHash_key
   ```
   Si manquante, exécuter `prisma/migrations/20260512_sms_log/migration.sql`
3. Vérifier aussi pourquoi `createNotification` (dedup au niveau Notification dans `reminders` cron) n'a pas bloqué le 2e et 3e SMS — peut-être que le cron a tourné 3× malgré `acquireCronLock` ? À investiguer dans Vercel cron logs.

### 2. ⛔ Vercel build red — `d6efcf9`
Le push est rouge mais erreur exacte non récupérée. Suspects par ordre de probabilité :

1. **`src/app/api/admin/backups/restore/[date]/route.ts` ligne 91** :
   ```ts
   const result = await (prisma[prismaModel] as any).createMany({...});
   ```
   En `strict + noImplicitAny`, `prisma[prismaModel]` peut fail car `prismaModel` est typé comme union de littéraux (`as const` sur `RESTORE_ORDER`) et n'est pas explicitement `keyof PrismaClient`. **Fix** : caster `prisma` d'abord :
   ```ts
   const result = await ((prisma as unknown as Record<string, { createMany: (a: unknown) => Promise<{ count: number }> }>)[prismaModel]).createMany({...});
   ```
   Ou plus simple : `(prisma as any)[prismaModel].createMany(...)`.

2. **`src/app/api/admin/diagnostics/route.ts`** — la nouvelle `lastEmailSentIso()` utilise `getEmailQueue().getCompleted(0, 0)`. BullMQ typings peuvent demander `getCompleted(start: number, end: number, asc?: boolean)` — vérifier l'usage. Si erreur de typage, fix : `getCompleted(0, 0, false)`.

3. **`src/app/[locale]/admin/health/HealthClient.tsx`** — le nouveau `isNeverAnomaly` est calculé avant `isNever`/`isOverdue` ; ordre de déclaration à vérifier. Peu probable mais possible.

4. **`src/app/[locale]/admin/backups/BackupsClient.tsx`** — l'import `UploadCloud` de lucide-react doit exister (il existe bien).
