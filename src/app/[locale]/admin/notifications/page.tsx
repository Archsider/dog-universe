...
import {
  CheckCheck, Loader2, BellOff, Bell,
  UserPlus, CalendarClock, CheckCircle2, XCircle, ArrowRight,
} from 'lucide-react';
...
const TYPE_CONFIG: Record<string, { icon: React.ElementType; color: string; bg: string }> = {
  NEW_CLIENT:          { icon: UserPlus,      color: 'text-blue-600',   bg: 'bg-blue-50' },
        ...
        <div className="space-y-2">
          {notifications.map((n) => {
            ...
            return (
              <div
                key={n.id}
                className={`bg-white rounded-xl border p-4 transition-all cursor-pointer ${n.read ? 'border-ivory-200' : 'border-gold-300 bg-gold-50/20'}`}
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      {!n.read && <span className="flex-shrink-0 w-2 h-2 rounded-full bg-gold-500 mt-1.5" />}
                    </div>
                    ...
                    <div className="flex items-center justify-between mt-2">
                      ...
                      {n.type === 'NEW_CLIENT' && meta.clientId && (
                        <Link
