...
export default async function BoardPage({ params }: { params: Promise<Params> }) {
  ...
  const bookings = await prisma.booking.findMany({
    ...
    include: {
      ...
      taxiTrips: { select: { id: true, tripType: true, status: true, address: true } },
    },
  ...
  const serialized = bookings.map((b) => ({
    ...
    taxiGoStatus: b.taxiTrips?.find(t => t.tripType === 'OUTBOUND')?.status ?? null,
    ...
    taxiReturnStatus: b.taxiTrips?.find(t => t.tripType === 'RETURN')?.status ?? null,
    taxiGoTripId: b.taxiTrips?.find(t => t.tripType === 'OUTBOUND')?.id ?? null,
    taxiReturnTripId: b.taxiTrips?.find(t => t.tripType === 'RETURN')?.id ?? null,
    standaloneTripId: b.taxiTrips?.find(t => t.tripType === 'STANDALONE')?.id ?? null,
    standaloneTripStatus: b.taxiTrips?.find(t => t.tripType === 'STANDALONE')?.status ?? null,
    taxiGoAddress: b.taxiTrips?.find(t => t.tripType === 'OUTBOUND')?.address ?? b.boardingDetail?.taxiGoAddress ?? null,
    taxiReturnAddress: b.taxiTrips?.find(t => t.tripType === 'RETURN')?.address ?? b.boardingDetail?.taxiReturnAddress ?? null,
    standaloneTripAddress: b.taxiTrips?.find(t => t.tripType === 'STANDALONE')?.address ?? null,
